...
import 'package:healthpocket/features/care/presentation/non_partner_payment_screen.dart';
...
class _FindCareScreenState extends State<FindCareScreen> {
  ...
  void _open(CareProvider provider) => Navigator.push(
    context,
    MaterialPageRoute<void>(
      builder: (_) => _ProviderDetails(
        provider: provider,
        providers: _providers,
        enabled: widget.demoRequestsEnabled,
        savingsStore: widget.savingsStore,
      ),
    ),
  );
  @override
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              const Text(
                'Partnered care, closer to you.',
                style: TextStyle(color: AppColors.inkMuted),
              ),
              const SizedBox(height: AppSpacing.md),
              _LocationCard(
                status: _locationStatus,
                onChanged: (value) => setState(() => _locationStatus = value),
              ),
              const SizedBox(height: AppSpacing.md),
              TextField(
                controller: _search,
                ...
                decoration: const InputDecoration(
                  hintText: 'Search hospitals, clinics or pharmacies...',
              ...
              if (_loading)
                ...
                if (partners.isEmpty)
                  const Padding(
                    ...
                    child: Text(
                      'No HealthPocket partners match this filter.',
                      style: TextStyle(color: AppColors.inkMuted),
                    ),
                  )
                else
                  ...partners.map(
                    (item) => _ProviderTile(
                      provider: item,
                      showDistance:
                          _locationStatus == DemoLocationStatus.granted,
                      onTap: () => _open(item),
                    ),
                  ),
                if (other.isNotEmpty) ...[
                  const SizedBox(height: AppSpacing.lg),
                  const _Heading('Other Healthcare Providers'),
                  const SizedBox(height: AppSpacing.sm),
                  ...other.map(
                    (item) => _ProviderTile(
                      provider: item,
    ...
    child: Column(
      ...
      children: [
        ...
        if (status == DemoLocationStatus.notRequested)
          ...
          Text(
            _locationMessage(status),
            style: const TextStyle(color: AppColors.inkMuted, fontSize: 12),
          ),
      ],
    ),
  );
  void _options(BuildContext context) => showModalBottomSheet<void>(
    context: context,
        ...
        child: Wrap(
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                onChanged(DemoLocationStatus.granted);
                Navigator.pop(context);
              },
            ),
            ListTile(
              title: const Text('Simulate denied'),
              onTap: () {
                onChanged(DemoLocationStatus.denied);
                Navigator.pop(context);
              },
            ),
            ListTile(
              title: const Text('Simulate unavailable'),
              onTap: () {
                onChanged(DemoLocationStatus.unavailable);
                Navigator.pop(context);
              },
            ),
          ],
...
String _locationMessage(DemoLocationStatus status) => switch (status) {
  DemoLocationStatus.granted =>
    'Demo location enabled · distances use mock coordinates.',
  DemoLocationStatus.denied =>
    'Location permission denied. Search and filters still work.',
  DemoLocationStatus.unavailable =>
    'Location is unavailable. Search and filters still work.',
  DemoLocationStatus.notRequested => 'Location has not been requested.',
};

class _FilterChips extends StatelessWidget {
  const _FilterChips({required this.selected, required this.onSelected});
  final CareProviderType? selected;
  final ValueChanged<CareProviderType?> onSelected;
  @override
  Widget build(BuildContext context) => SizedBox(
    ...
    child: ListView(
      ...
      children: [
        ...
        _Filter(
          label: 'Hospitals',
          active: selected == CareProviderType.hospital,
          onTap: () => onSelected(CareProviderType.hospital),
        ),
        _Filter(
          label: 'Clinics',
          active: selected == CareProviderType.clinic,
          onTap: () => onSelected(CareProviderType.clinic),
        ),
        _Filter(
          label: 'Pharmacies',
          active: selected == CareProviderType.pharmacy,
          onTap: () => onSelected(CareProviderType.pharmacy),
        ),
        _Filter(
          label: 'Diagnostics',
          active: selected == CareProviderType.diagnostics,
          onTap: () => onSelected(CareProviderType.diagnostics),
        ),
      ],
    ),
  );
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${_typeName(provider.type)}${showDistance ? ' · ${provider.mockDistanceKm.toStringAsFixed(1)} km' : ''}',
                      style: const TextStyle(
                        color: AppColors.inkMuted,
                        fontSize: 12,
                      ),
                    ...
                    Row(
                      children: [
                        _OpenPill(open: provider.isOpen && provider.active),
...
class _Logo extends StatelessWidget {
  const _Logo({required this.provider, this.large = false});
  final CareProvider provider;
  final bool large;
  @override
  Widget build(BuildContext context) {
    final icon = switch (provider.type) {
      ...
      CareProviderType.hospital => LucideIcons.building2,
      CareProviderType.clinic => LucideIcons.stethoscope,
      CareProviderType.diagnostics => LucideIcons.flaskConical,
    };
    return Container(
      width: large ? 66 : 48,
      height: large ? 66 : 48,
      ...
      child: Icon(icon, color: AppColors.primary, size: large ? 29 : 22),
    );
  }
}
...
class _OpenPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      color: open ? const Color(0xFFE3F7EB) : AppColors.secondarySoft,
      borderRadius: BorderRadius.circular(10),
    ),
    child: Text(
      open ? 'Open' : 'Closed',
      style: TextStyle(
        color: open ? AppColors.success : AppColors.secondary,
        fontWeight: FontWeight.w700,
        fontSize: 10,
      ),
    ),
...
class _Badge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Row(
    mainAxisSize: MainAxisSize.min,
    children: [
      Icon(LucideIcons.badgeCheck, color: AppColors.primary, size: 14),
      SizedBox(width: 3),
      Text(
        'HealthPocket Partner',
        style: TextStyle(
        ...
        children: [
          Container(
            ...
            decoration: const BoxDecoration(
              gradient: LinearGradient(
                colors: [Color(0xFFBEEDEA), AppColors.primary],
              ),
            ),
            child: Stack(
              children: [
                ...
                const Positioned(
                  right: 54,
                  top: 6,
                  child: _RoundButton(icon: LucideIcons.heart),
                ),
                const Positioned(
                  right: 6,
                  top: 6,
                  child: _RoundButton(icon: LucideIcons.share2),
                ),
                const Center(
                  child: Icon(
                    LucideIcons.cross,
            ...
            child: ListView(
              ...
              children: [
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _Logo(provider: provider, large: true),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${_typeName(provider.type)} · ${provider.city}',
                ...
                const SizedBox(height: AppSpacing.md),
                Text(
                  '${provider.isOpen && provider.active ? 'Open' : 'Closed'}${provider.closingTime == null ? '' : ' · Closes ${provider.closingTime}'}',
                  style: TextStyle(
                    color: provider.isOpen && provider.active
                        ? AppColors.success
                        : AppColors.secondary,
                    fontWeight: FontWeight.w700,
                  ),
                ...
                const Row(
                  children: [
                    Expanded(
                      child: _Action(icon: LucideIcons.phone, label: 'Call'),
                    ),
                    SizedBox(width: 8),
                    Expanded(
                      child: _Action(
                    ...
                    SizedBox(width: 8),
                    Expanded(
                      child: _Action(icon: LucideIcons.heart, label: 'Save'),
                    ),
                  ],
                ),
                const SizedBox(height: AppSpacing.lg),
                ...
                const SizedBox(height: AppSpacing.xs),
                Text(
                  provider.description ?? 'Fictional demo provider. No booking, payment, or real care authorization is available. This is not evidence of a partnership.',
                  style: const TextStyle(color: AppColors.inkMuted),
                ),
                if (provider.isDemo)
                  const Padding(
                    ...
                    child: Text(
                      'No booking, payment, or real care authorization is available in this beta directory.',
          ...
          Padding(
            ...
            child: AppPrimaryButton(
              label: provider.isPartner
                  ? 'Pay ${provider.name}  →'
                  : 'Pay with HealthPocket  →',
              onPressed: !provider.active
                  ? null
                  : () => Navigator.push(
                      context,
                      MaterialPageRoute<void>(
                        builder: (_) => provider.isPartner
                            ? DemoCareJourneyScreen(
                                providers: providers,
                                selectedProviderId: provider.id,
                                savingsStore: savingsStore,
                              )
                            : NonPartnerPaymentScreen(
                                provider: provider,
                                savingsStore: savingsStore,
                              ),
                      ),
...
String _typeName(CareProviderType type) => switch (type) {
